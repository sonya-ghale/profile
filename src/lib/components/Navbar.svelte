...
>
	<div class="mx-auto flex max-w-6xl flex-col items-center gap-3 sm:gap-4 md:flex-row md:justify-between">
		<ul
			class="m-0 flex w-full list-none flex-wrap justify-center gap-x-4 gap-y-2 p-0 sm:gap-x-5 md:w-auto md:justify-start md:gap-8"
		>
			{#each navLinksLeft as link}
				<li>
					<a
						href={link.href}
						class="text-sm font-semibold uppercase tracking-wide text-muted no-underline transition-colors hover:text-accent sm:text-base"
		...
		<div class="flex items-center gap-4 sm:gap-5 sm:items-right">
			{#each navLinksRight as link}
				<a
					...
					class="flex items-center gap-2 text-sm font-semibold uppercase tracking-wide text-muted no-underline transition-colors hover:text-accent sm:text-base"
